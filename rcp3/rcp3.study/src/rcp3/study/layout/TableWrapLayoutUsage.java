...
public class TableWrapLayoutUsage implements ShellRunner {
  ...
  public void fillContent(Composite parent) {
    ...
    label = toolkit.createLabel(form.getBody(), "This text will span two rows and should not grow the column.",
        SWT.WRAP);
    td = new TableWrapData();
    td.rowspan = 2;
    label.setLayoutData(td);
    ...
    label = toolkit.createLabel(form.getBody(), "This text goes into column 3 and consumes only one cell too",
        SWT.WRAP);
